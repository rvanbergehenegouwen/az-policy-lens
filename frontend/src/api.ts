...
export const apiClient = {
  ...
  // Policy endpoints
  getPolicies: (params?: { skip?: number; limit?: number }) =>
    api.get('/api/policies', { params }),
  getPolicyById: (id: string) => api.get(`/api/policies/${id}`),
  searchPolicies: (query: string) => api.get('/api/policies/search', { params: { q: query } }),
  
  // User endpoints
  getCurrentUser: () => api.get('/api/me'),
  getUsers: () => api.get('/api/users'),
  createUser: (email: string, role: string) =>
    api.post('/api/users', { email, role }),
  ...
  // Audit endpoints
  getAuditLogs: (params?: { skip?: number; limit?: number }) =>
